module Forge
  class Camera < Process
    ...
    def px_wid
      (Scaler.viewport_width / Forge.config.game_class::SCALE / zoom).ceil
    end

    def px_hei
      (Scaler.viewport_height / Forge.config.game_class::SCALE / zoom).ceil
    end
    ...
    def on_screen_case?(cx, cy, padding = 32)
      cx * Forge.config.game_class::GRID >= px_left - padding && (cx + 1) * Forge.config.game_class::GRID <= px_right + padding &&
      cy * Forge.config.game_class::GRID >= px_top - padding && (cy + 1) * Forge.config.game_class::GRID <= px_bottom + padding
    end
    ...
    def level_to_global_x(v)
      scroller = Forge.config.game_class.s.scroller
      scale = Forge.config.game_class::SCALE * zoom
      # scroller.x is already scaled, so divide it back, then add level position
      (v + scroller.x / scale) * scale
    end

    def level_to_global_y(v)
      level = Forge.config.game_class.s.current_level
      scroller = Forge.config.game_class.s.scroller
      scale = Forge.config.game_class::SCALE * zoom
    ...
    def apply
      ...
      #scaling
      scroller.x *= Forge.config.game_class::SCALE * zoom
      scroller.y *= Forge.config.game_class::SCALE * zoom

      scroller.x = scroller.x.round
      scroller.y = scroller.y.round

      scroller.scale = Forge.config.game_class::SCALE * zoom
    end
    ...
    def c_wid
      (px_wid / Forge.config.game_class::GRID).ceil
    end

    def c_hei
      (px_hei / Forge.config.game_class::GRID).ceil
    end
    ...
    def c_left
      (px_left / Forge.config.game_class::GRID).to_i
    end

    def c_right
      (px_right / Forge.config.game_class::GRID).to_i
    end

    def c_top
      (px_top / Forge.config.game_class::GRID).to_i
    end

    def c_bottom
      (px_bottom / Forge.config.game_class::GRID).to_i
    end
  end
end

module Forge
  class Game < Process
    ...
    def initialize
      ...
      # Auto-spawn player if a Player class is defined
      ensure_player_spawned if auto_spawn_player?
    ...
    def auto_load_map
      if $gtk.stat_file(map_path)
        @root = Forge::Ldtk::Root.import($gtk.parse_json_file(map_path))
      end
    end
    ...
    def spawn_ldtk_entities(level, force = false)
      ...
      level.layer("Entities").entity_instances.each do |ldtk_entity|
        entity_class = Forge::Entity.resolve(ldtk_entity.identifier)
    ...
    # Convenience method - spawns the player if not already spawned
    # Can be overridden in subclasses for custom player setup
    def ensure_player_spawned
      return player if player&.spawned?
      
      player_class = Forge::Entity.resolve("Player")
      if player_class
        user.spawn_player(player_class)
      else
        # Try to auto-create a basic player if no Player class is defined
        create_default_player
      end
    end

    # Override this method in subclasses to customize default player
    def create_default_player
      return nil
    end

    # Override to disable auto-spawning of player
    def auto_spawn_player?
      true
    end

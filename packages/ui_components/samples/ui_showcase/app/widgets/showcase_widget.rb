# ShowcaseWidget -- demonstrates every Forge::Ui component
#
# This is a single widget that builds the entire showcase UI using
# the DSL methods built into Forge::Widget: panel, label, button,
# text_input, checkbox, toggle, slider, progress_bar, dropdown,
# radio_group, row, and image.

class ShowcaseWidget < Forge::Widget
  def init
    @status = "Welcome to the Forge UI Showcase"
    @counter = 0
    @progress_val = 0
    @typed_text = ""
    @volume = 75
  ...
  def render_title
    label :title,
      x: 640, y: 700,
      text: "Forge UI Component Showcase",
      size_key: :size_xl,
      color_key: :accent,
      align: :center
  end
  ...
  def render_toggles_section
    panel :toggles_panel, x: 20, y: 175, w: 370, h: 230, title: "Checkboxes & Toggles" do
      ...
      toggle :tgl_dark,
        ...
        on_change: ->(t) {
          @dark_mode = t.on
          $forge_ui_theme = t.on ? Forge::Ui::Theme.new : Forge::Ui::Theme.light
    ...
    panel :quest_panel, x: 1040, y: 175, w: 220, h: 230, title: "Quests Demo" do
      ...
      button :btn_dive, text: "Progress Treasure", w: 190, size: :sm, style: :primary,
             on_click: ->(b) {
               ...
               if q && !q.complete?
                 qm.progress(:find_treasure)
                 @status = "#{q.name} complete!"
               else
      ...
      button :btn_mine, text: "Mine Ore", w: 190, size: :sm, style: :warning,
             on_click: ->(b) {
               qm = entity.quest_script.manager
               qm.progress(:craft_sword)
               @status = "Crafting quest progress!"
             }
  ...
  def render_status_bar
    t = $forge_ui_theme || Forge::Ui::Theme.new
